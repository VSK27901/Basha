...
from app.models.models import UserResetPass, UserForgotPassword, User, UserDeviceData
...
async def verification(request: Request, token: str = Query(...)):
    try:
        ...
        users_collection.update_one({"email": verification_data["email"]}, {"$set": {"verified": True}})
...
async def resetpassword(request: Request, token: str):
    try:
        ...
        reset_password_data = verification_collection.find_one({"reset_password_token": token})
...
async def myaccount(request: Request, current_user: dict = Depends(get_current_user_from_cookie)):
    ...
    try:
        ...
        #     # Handle user data not found, redirect or raise an exception
        ...
        return templates.TemplateResponse("adminacc.html", {"request": request, "username": username, "email": email, "role": role })
...
async def myshipment(request: Request, current_user: dict = Depends(get_current_user_from_cookie)):
    try:
        ...
        user_shipments = shipments_collection.find({"username": current_user["username"]})
...
async def logout(request: Request, access_token: str = Cookie(None)):
    try:
        ...
        clear_access_token_cookie(response)  # Call the function to clear the access token cookie
        return response
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal Server Error: {str(e)}")
